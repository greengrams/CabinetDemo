...
def GetFeatures(data,trainingLabel,dataPool,en_params):
    ...
    for idx in range(len(dataPool[0])):
        ...
        envelopeResult = np.array(envelope(np.array(trainingLabel[idx*len(tmp):(idx+1)*len(tmp)]), tmp, data[idx].tolist(), axis_en_params))
        if len(en_params)<=idx:
            en_params.append(axis_en_params)
        features = np.insert(features, features.shape[1], envelopeResult.T, axis=1)
    return features
def TrainingByParams(dataPool, trainingLabel,params,en_params):
    features = []
    labels = []
    rangeOfData = [0]
    modelPool = []
    ...
    for i in xrange(len(set(labels))):
        ...
        out = open(filename, 'w')
        out.write(','.join([str(x) for x in features[labels==i]]) + '\n')
...
def LoadTrainingData(namelist):
    ...
    return dataPool, trainingLabel

def Train(namelist,params,en_params):
    dataPool, trainingLabel= LoadTrainingData(namelist)
    ## Training Model
    modelPool, p_pool, p_table, testingData, _, scaleRange, scaleMin, rangeOfData, LogRegPool, logis_prob, p_min_table = TrainingByParams(dataPool, trainingLabel,params,en_params)
