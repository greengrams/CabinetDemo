...
def Vectorize(seriesData):
    ...
    vectorFeature[:, 10] = np.median(seriesData[:, 0:int(np.floor(seriesData.shape[1] * 0.10))], 1)

    segLenth = float(len(seriesData[0])) / 5.0

    for idx in range(5):
        vectorFeature[:, int(11+idx)] = np.mean(seriesData[:, int(np.floor(segLenth * idx)):int(np.floor(segLenth * (idx+1)))], 1)
